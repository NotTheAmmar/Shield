...
graph TD
    ...
    Ledger -->|JSON-RPC| Blockchain[(EVM Blockchain)]
```
...
| **EVM Blockchain** | `shield-ledger` | `Chain ID 31337` | Append-only, cryptographically verified ledger for evidence SHA-256 hashes. | `Ethereum PoA` (Clique / Merkle Trees) |
...
| `ledger_tx_id` | `TEXT` | | | Transaction ID generated by the EVM Blockchain. |
| `ledger_timestamp`| `TIMESTAMPTZ` | | | Precise timestamp when the blockchain acknowledged the hash. |
...
## 3. EVM Blockchain Ledger Schema

In SHIELD, `shield-ledger` acts as the interface layer. It enforces a strict schema standard on the smart contract:
...
2. It calls the Ledger Service (`shield-ledger`), which queries the smart contract using the key `evidenceId`.
3. The blockchain returns the **original record value** along with a cryptographic **transaction ID (TxID)**.
