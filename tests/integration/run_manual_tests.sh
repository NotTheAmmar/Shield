#!/bin/bash
echo "ðŸ›¡ï¸ Starting SHIELD Backend Master Evaluation Suite (Bash/cURL)..."
...
if [ -z "$ADMIN_TOKEN" ]; then echo "âŒ FAILED: $BODY"; rm -f "$TEST_FILE"; exit 1; else echo "âœ… JWT Issued"; fi
...
if [ -z "$OFFICER_EMAIL" ]; then echo "âŒ FAILED: $BODY2"; rm -f "$TEST_FILE"; exit 1; else echo "âœ… User Created: $OFFICER_EMAIL"; fi
...
if [ -z "$OFFICER_TOKEN" ]; then echo "âŒ FAILED: $BODY3"; rm -f "$TEST_FILE"; exit 1; else echo "âœ… Officer JWT Issued"; fi
...
if [ -z "$FIR_ID" ]; then echo "âŒ FAILED: $BODY4"; rm -f "$TEST_FILE"; exit 1; else echo "âœ… FIR Created: $FIR_ID"; fi
...
if [ -z "$EVIDENCE_ID" ]; then echo "âŒ FAILED: $BODY5"; rm -f "$TEST_FILE"; exit 1; else echo "âœ… Uploaded! ID: $EVIDENCE_ID"; fi
...
if [ "$STATUS" != "OK" ]; then echo "âŒ FAILED: $BODY6"; rm -f "$TEST_FILE"; exit 1; else echo "âœ… Cryptographic Verification Successful"; fi
...
if [ -z "$TOTAL_EVIDENCE" ]; then echo "âŒ FAILED: $BODY7"; rm -f "$TEST_FILE"; exit 1; else echo "âœ… Stats Active. Total Evidence: $TOTAL_EVIDENCE"; fi
